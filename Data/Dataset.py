...
class DATATRAIN(MVTec3D):
    ...
    def __getitem__(self, idx):
        ...
        organized_pc = Util.read_tiff_organized_pc(tiff_path)
        depth_map_3channel = np.repeat(
            Util.organized_pc_to_depth_map(organized_pc)[:, :, np.newaxis], 3, axis=2
        )
        resized_depth_map_3channel = Util.resize_organized_pc(depth_map_3channel)
        resized_organized_pc = Util.resize_organized_pc(organized_pc)
...
def DATALOADER(split, img_size):
    """DataLoader fixed for 'rope' only"""
    if split == 'train':
